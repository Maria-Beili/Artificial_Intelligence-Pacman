...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, starting_game_state, corners=None):
        ...
        "*** YOUR CODE HERE ***"
        # Since a parameter is passed, it must be initialized to the attribute of the class
    ...
    def get_start_state(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # return the starting position and the corners that have been visited
    ...
    def is_goal_state(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # state[1] is the variable with the corners that have been visited not 
        # return True if there are no more corners to visit because if there is no corner left, it means that all corners have been visited and reached our goal
        # False if there are still corners to visit
    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # Get the next position if the action is taken
            x, y = current_position
            ...
            if not hits_wall:
                ...
                # Get the corners that have been visited
                corners = list(state[1])

                # Remove the corner if the new position is a corner, else keep the corners as they are, to avoid visiting the same corner twice
                if new_position in corners:
                    corners.remove(new_position)
                
                # And then create a new state with the new position and the new corners. After append the new state to the successors list
                # to update the successors list. In the successor's list we store the states that can be reached from the current state
                new_state = new_position, tuple(corners)
...
def corners_heuristic(state, problem):
    ...
    # For each corner find the nearest corner to the current position and add the distance to the heuristic.
    # Is admisible because never overestimates the cost of reaching the goal, that is, the distance to the nearest corner is always less than or equal to the actual distance to the corner.
    heuristic = 0

    while corners:
        # The function nearest_corner returns the distance to the nearest corner and it's position
        # The function nearest_corner works by calculating the Manhattan distance between the current position and each corner of the list and returns the minimum distance and the position of the nearest corner
        dist, position = nearest_corner(position, corners)

        # Add the distance to the heuristic
        heuristic += dist

    # Return the heuristic thar approximates the cost of reaching the goal
    return heuristic
...
def food_heuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # Define the heuristic as the maximum distance to the nearest corner
    heuristic = []

    # If there are no food dots, return 0
    if food_grid.count() == 0: return 0

    # Otherwise, return the Manhattan distance between the current position and the food dot
    else:
        # Find the corners using the find_corners function, where each corner only considers points of its own quadrant, which is more efficient because it reduces the number of points to consider
        corners = find_corners(food_grid)

        # For each corner, calculate the distance between the current position and a food dot inside a maze (taking into account walls)
        # and store the maximum distance that represents the worst case scenario to reach the food dot
        # which represents that this heuristic is admissible because it never overestimates the cost of reaching the goal
        for corner in corners:
            heuristic.append(maze_distance(position, corner, problem.starting_game_state))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def find_path_to_closest_dot(self, game_state):
        ...
        "*** YOUR CODE HERE ***"
        # Solving the problem using BFS because it is faster than A* for this problem
        # BFS is faster than A* because the heuristic is not needed for this problem, because the goal is to find the closest dot, not the optimal path
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # return True if the current position is a food dot and False otherwise
